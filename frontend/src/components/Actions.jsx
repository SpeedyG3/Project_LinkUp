...
const Actions = ({post}) => {
	...
	const handleLikeAndUnlike = async () => {
		...
		try {
			...
			if (!liked) {
				//add id of curr user to post.likes array
				...
				//remove id of curr user from post.likes array
				const updatedPosts = posts.map((p) => {
					if(p._id === post._id){
	...
	const handleReply = async () => {
		if(!user){
			return toast("Error", "You need to be logged in to reply to a post", "error");
		}
		if(isReplying){
			return;
		}
