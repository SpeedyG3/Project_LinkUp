import { Avatar, Box, VStack, Text, Flex, Link, useToast } from "@chakra-ui/react"
...
const UserHeader = () => {
    ...
    const copyUrl = () => {
        ...
        navigator.clipboard.writeText(currurl).then(() => {
            toast({
                description: "Profile Copied to Clipbaord!",
                status: "success",
                duration: 2000,
                isClosable: true,
            });
    ...
    return (<VStack gap={4} alignItems={"start"}>
        ...
        <Flex w={"full"}>
            ...
            <Flex flex={1} borderBottom={"1px solid gray"}
                justifyContent={"center"} color={"gray.light"} pb="3" cursor={"pointer"}>
                <Text fontWeight={"bold"}>
                    Replies
                </Text>
            </Flex>
